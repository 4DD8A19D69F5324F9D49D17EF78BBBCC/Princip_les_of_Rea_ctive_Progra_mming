...
class Replica(val arbiter: ActorRef, persistenceProps: Props) extends Actor {
  ...
  case class GlobalAck(src: ActorRef,initialWaiting: Set[ActorRef],successmsg: Any,persistmsg :Any,failedmsg: Any = Nil) {
    var waiting : Set[ActorRef] = initialWaiting
    def acked = waiting.isEmpty
    def completed(actor : ActorRef) = {
      waiting -= actor
    }
    def retry = {
    ...
    def completeAndSent = if(acked) {
      src ! successmsg 
      true
    }else false
  }
  
  var acks = Map.empty[String,GlobalAck] // key => acks 
  
  def waitingForAck(key:String,src:ActorRef,initialWaiting: Set[ActorRef], successmsg:Any,persistmsg: Any,failedmsg:Any=Nil,hasfail:Boolean=false){
  ...
  val leader: Receive = {
    case Insert(key,value,id) => 
      ...
      waitingForAck(key, sender, replicators+persistence, OperationAck(id), Persist(key,Some(value),id),OperationFailed(id),true)
    case Remove(key,id) =>
      kv-=key
      persistence ! Persist(key,None,id)
      secondaries.values foreach{_ ! Replicate(key,None,id)}
      waitingForAck(key, sender, replicators+persistence, OperationAck(id), Persist(key,None,id),OperationFailed(id),true)
    ...
    case Replicas(replicas) => {
      ...
      added foreach {
        replica => 
          ...
          kv foreach{case (k,v) => 
            replicator ! Replicate(k,Some(v),Random.nextLong)
          }
  ...
  val replica: Receive = {
    case Snapshot(key,valueoption,id) =>  if(id>seqNo) {
